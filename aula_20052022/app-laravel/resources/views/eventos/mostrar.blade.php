    ...
    <div class="col-md-10 offset-md-1">
        <div class="row">
            <div id="image-container" class="col-md-6">
                <img src="/img/eventos/{{ $evento->imagem }}" class="img-fluid" {{ $evento->titulo }}>
            </div>
            <div id="info-container" class="col-md-6">
                ...
                <p class="events-participants">
                    <ion-icon name="people-outline"></ion-icon> {{ count($evento->users) }} participante(s)
                </p>
                <p class="event-owner">
                ...
                @if (!$hasUserJoined)
                    <form action="/eventos/join/{{ $evento->id }}" method="GET">
                        @csrf
                        <a href="/eventos/join/{{ $evento->id }}" class="btn btn-primary" id="event-submit"
                            onclick="evento.preventDefault(); this.closest('form').submit();"> Confirmar Presença </a>
                    </form>
                @else
